...
class HamsterCombat():
    ...
    def find_best_upgrades(self, upgrades, current_balance, time_horizon=2):
        ...
        for x in best_upgrades:
            print('[~] Upgrade id: ', x['id'], f"| Return after {time_horizon} days: {x['x_day_return']} | Price: {x['price']}")
    ...
    def best_upgrades(self):
        ...
        print('[+] Avabile Upgrades:  ', len(sorted_upgrades))
        ...
        for upgrade_to_buy in sorted_upgrades:
            ...
            if 'cooldownSeconds' in upgrade_to_buy and upgrade_to_buy['cooldownSeconds'] > 0:
                print('[~] Continue update for cool down:  ', upgrade_to_buy['id'])
                continue
            ...
            if upgrade_to_buy['condition'] != None and upgrade_to_buy['condition']['_type'] == 'ByUpgrade':
                ...
                if xlevel > ulevel:
                    if balance < upgrade_to_buy['price']:
                        continue
                    print('[~] Updating: ', upgrade_to_buy['id'], ' | x_day_return: ', upgrade_to_buy['x_day_return'])
                    response = self.buy_upgrade(upgrade_to_buy['id'])
                    ...
                    for i in range((ulevel-xlevel)+1):
                        if balance < xprice:
                            break
                        print('[~] Updating: ', uid, ' | Need for: ', upgrade_to_buy['id'])
    ...
    def auto_tap(self):
        ...
        while availableTaps > 10:
            ...
            if x > availableTaps:
                x = availableTaps
            print(f'[~] Tapping {x} Times')
            taps = self.tap(x, availableTaps)
            availableTaps = taps['clickerUser']['availableTaps']
            balanceCoins = taps['clickerUser']['balanceCoins']
            print('[+] Available Taps: ', availableTaps)
            print('[+] Balance Coins: ', round(balanceCoins, 3))
